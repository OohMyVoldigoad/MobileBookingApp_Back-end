...
export default function Company(props) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://10.170.8.184:8000/api/penyedia_lapangan/'+ item.id +'/jenisLapangan');
...
return (
    ...
    <View style={styles.modalDialog}>
        ...
        {/* destination image */}
        <Image source={{ uri:'http://10.170.5.149:8000/storage/'+ item.foto }} style={{width: wp(100), height: hp(25)}} />
        ...
        <View style={{borderTopLeftRadius: 40, borderTopRightRadius: 40}} className="px-5 flex flex-1 justify-between bg-[#FFF9E8] pt-8 -mt-14">
            <ScrollView showsVerticalScrollIndicator={false} className="space-y-1">
                ...
                >
                    <Image source={{ uri:'http://10.170.5.149:8000/storage/'+ item.foto }} style={{width: wp(84), height: hp(25)}} className="flex mx-1 mb-4 bg-[#BCD8A6] rounded-3xl p-1 pl-1"/>
                    <View
                        style={{
                            alignItems: 'center',
                        }}
